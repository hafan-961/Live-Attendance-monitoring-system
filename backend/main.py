...
# # IMPORTANT: Update this IP every time you restart IP Webcam on your phone
# CAMERA_URL = "http://10.169.91.52:8080/video" 
...
#             if not dest_email or "@" not in dest_email: continue
#             msg = MIMEMultipart()
#             msg['From'] = SENDER_EMAIL; msg['To'] = dest_email; msg['Subject'] = f"Absence Alert: {name}"
#             body = f"Dear Parent,\n\nYour ward, {name} (Reg No: {reg_no}), was absent for today's session.\n\nRegards,\nAttendance System"
...
# def start_session():
#     global is_session_active, attendance_records, last_marked_time
...
#             if str(reg_no) in present_ids: present_list.append(data)
...
#         return jsonify({"status": "success", "message": f"Session Saved. Emails triggered."})
...
#             if frame is None: 
#                 time.sleep(0.1) # Prevent CPU freezing
#                 continue
...
# app_flask = Flask(__name__)
# # Added DELETE to allowed methods for CORS
# CORS(app_flask, resources={r"/*": {"origins": "*"}}, methods=["GET", "POST", "DELETE", "OPTIONS"])
...
# # --- AI PROCESSING THREAD (IDENTITY + EMOTION) ---
# def processing_thread_func():
#     global current_faces_data, attendance_records, last_marked_time, class_sentiment, is_session_active
#     while True:
#         frame_to_process = None
...
#                     # 2. IDENTITY RECOGNITION (INSIGHTFACE)
#                     embedding = face.embedding / np.linalg.norm(face.embedding)
#                     name, color = "Unknown", (0, 0, 255)
#                     with embeddings_lock:
#                         if registered_embedding_vectors.size > 0:
...
#                                 # --- RECORD ATTENDANCE + EMOTION ---
#                                 if is_session_active:
#                                     if name not in last_marked_time or (time.time() - last_marked_time[name]) > 30:
#                                         attendance_records[name] = {
#                                             'status': 'Present', 
#                                             'timestamp': datetime.now().isoformat(),
#                                             'emotion': emotion_label # <--- STORE INDIVIDUAL EMOTION
#                                         }
#                                         last_marked_time[name] = time.time()

#                     processed_results.append({"bbox": [x1,y1,x2,y2], "name": name, "color": color, "emotion": emotion_label})
            
#             # 3. CLASS SENTIMENT CALCULATION
#             if frame_vibe_scores:
#                 avg = sum(frame_vibe_scores) / len(frame_vibe_scores)
#                 sentiment_history.append(avg); smoothed = sum(sentiment_history) / len(sentiment_history)
#                 class_sentiment = {"rating": "Excellent" if smoothed > 80 else "Focused" if smoothed > 60 else "Bored", "score": int(smoothed), "count": len(faces)}
...
#         if absent_list: threading.Thread(target=send_email_thread, args=(absent_list,), daemon=True).start()
#         is_session_active = False; attendance_records.clear(); last_marked_time.clear()
#         return jsonify({"status": "success", "message": f"Session Saved. Emails triggered for {len(absent_list)} students."})
#     except Exception as e: return jsonify({"status": "error", "message": str(e)}), 500
